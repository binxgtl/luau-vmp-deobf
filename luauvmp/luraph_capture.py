...
def extract_interpreter_factory(vm_source: str) -> str:
    ...
    start = candidates[-1]
    names = [part.strip() for part in start.group(3).split(",") if part.strip()]
    if not names:
...
local function numberToken(value)
    if value ~= value then return "Dnan"
    elseif value == math.huge then return "Dinf"
    elseif value == -math.huge then return "D-inf" end
...
local function tableToken(value, seen, depth)
    if isProto(value) then
        ...
        if id == nil then id = registerProto(value, -2, -2, -2) end
        return "P" .. tostring(id)
    end
    if depth > 5 or seen[value] then return "T{Xcycle=Xcycle}" end
    seen[value] = true
    local entries = {}
    local count = 0
    for key, item in value do
        ...
        local keyToken
        if keyType == "number" then keyToken = numberToken(key)
        elseif keyType == "string" then keyToken = "S" .. hexString(key)
        elseif keyType == "boolean" then keyToken = key and "B1" or "B0"
        else keyToken = "X" .. keyType end
        local itemType = type(item)
        local itemToken
        if item == nil then itemToken = "N"
        elseif itemType == "boolean" then itemToken = item and "B1" or "B0"
        elseif itemType == "number" then itemToken = numberToken(item)
        elseif itemType == "string" then itemToken = "S" .. hexString(item)
        elseif itemType == "table" then itemToken = tableToken(item, seen, depth + 1)
        else itemToken = "X" .. itemType end
...
local function typed(value)
    ...
    if kind == "boolean" then return value and "B1" or "B0"
    elseif kind == "number" then return numberToken(value)
    elseif kind == "string" then return "S" .. hexString(value)
    elseif kind == "table" then return tableToken(value, {}, 0) end
    return "X" .. kind
end
...
local function writeRuntimeFacts(runtime)
    ...
    for i = 1, 64 do
        ...
        local text
        local class = ""
        if kind == "nil" then text = "nil"
        elseif kind == "string" then text = "hex:" .. hexString(value)
        elseif kind == "boolean" or kind == "number" then text = cleanText(value)
        else
...
local function capture(runtimeState, root)
    ...
    if not isProto(root) then error("capture received an invalid root prototype") end
    ...
    while cursor <= #protoList do
        ...
        for pc = 1, instructionCount do
            for _, field in operandFields do
                ...
                if isProto(value) then registerProto(value, id, field, pc) end
            end
        end
...
-- The loaded VM receives no require/fs/net/process/Roblox API.
local environment = {
    __LUAUVMP_CAPTURE = capture,
    assert = assert, error = error, getfenv = getfenv,
    getmetatable = getmetatable, ipairs = ipairs, next = next, pairs = pairs,
    pcall = pcall, rawequal = rawequal, rawget = rawget, rawlen = rawlen,
    rawset = rawset, select = select, setfenv = setfenv,
    setmetatable = setmetatable, tonumber = tonumber, tostring = tostring,
    type = type, unpack = unpack, xpcall = xpcall, bit32 = bit32,
    buffer = buffer, coroutine = coroutine, math = math, string = string,
    table = table, utf8 = utf8,
}
...
''' % (
        _lua_quote(patched_vm), _lua_quote(bytecode),
        _lua_quote(full_ir), _lua_quote(runtime_facts),
    )
...
def _normalise_runtime_command(runtime: Union[str, Sequence[str]]) -> list[str]:
    ...
    if len(command) == 1 and shutil.which(command[0]) is None:
        raise CaptureError("Lune was not found. Install Lune or pass --runtime /path/to/lune")
    return command
...
def run_capture(
    ...
    try:
        result = subprocess.run(
            command, cwd=str(work), text=True, capture_output=True,
